...
import com.shahrohit.hashcodex.queries.ProblemSubmissionQuery;
...
public interface ProblemSubmissionRepository extends JpaRepository<ProblemSubmission, Long> {
    @Query(ProblemSubmissionQuery.EXT_PROBLEM_SOLVED)
    boolean existsProblemSolved(
        @Param("userId") Long userId,
        @Param("problemId") Long problemId
    );

    @Query(ProblemSubmissionQuery.EXT_PROBLEM_ATTEMPTED)
    boolean existProblemAttempted(
        @Param("userId") Long userId,
        @Param("problemId") Long problemId
    );

    @Query(ProblemSubmissionQuery.FIND_SUBMISSIONS)
    ...
    @Transactional
    @Modifying
    @Query(ProblemSubmissionQuery.UP_SUBMISSION_STATUS)
    void updateSubmissionStatus(
        @Param("id") Long id,
        @Param("status") ProblemSubmissionStatus status
    );
}
